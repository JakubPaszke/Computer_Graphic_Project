...
void main()
{
    vec3 color = texture2D(texture, texCoords).xyz;

    //TU BLAD
    vec3 normal = normalize(vecNormal);
    //vec3 normal = normalize((texture2D(textureNormal, texCoords).xyz) * 2 - 1);
    //vec3 normal = vecNormal;
    
    float roughness = texture2D(textureArm, texCoords).y;
    float metallic = texture2D(textureArm, texCoords).z;
    ...
    //sun
	ilumination=ilumination+PBRLight(sunDir,sunColor*calculateShadow(normal, sunDir, sunSpacePos, depthMap),normal,viewDir,color,roughness,metallic);
